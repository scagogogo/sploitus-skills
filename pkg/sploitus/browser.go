...
func (bs *BrowserSearcher) Search(query string, searchType string, sort string, offset int) (*types.SearchResponse, error) {
	...
	// 等待CloudFlare验证完成（使用智能等待替代固定Sleep）
...
func (bs *BrowserSearcher) extractResultsFromPage(page *rod.Page) *types.SearchResponse {
	...
	for _, selector := range selectors {
		...
		if err == nil && len(resultItems) > 0 {
			...
			if selector == "a[href*='/exploit?id=']" {
				exploits := extractExploitsFromLinks(resultItems)
				if len(exploits) > 0 {
	...
	for _, item := range resultItems {
		exploit := extractExploitFromCard(item)
		if exploit.Title != "" {
...
// extractExploitsFromLinks 从链接元素列表提取漏洞信息
func extractExploitsFromLinks(items []*rod.Element) []types.Exploit {
	var exploits []types.Exploit
...
func extractExploitFromCard(item *rod.Element) types.Exploit {
	var exploit types.Exploit
